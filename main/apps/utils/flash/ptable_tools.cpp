    ...
    namespace FLASH_TOOLS
    {
        ...
        FlashStatus PartitionTable::loadFromFile(const std::string& filename)
        {
            ...
            else if (app_desc.magic_word == ESP_BOOTLOADER_MAGIC_WORD1 || app_desc.magic_word == ESP_BOOTLOADER_MAGIC_WORD2)
            {
                ...
                ESP_LOGD(TAG, "Successfully read %zu partitions from file %s", m_partitions.size(), filename.c_str());
                return FlashStatus::SUCCESS;
            }
        ...
        bool PartitionTable::makeDefaultPartitions()
        {
            ...
            ESP_LOGD(TAG, "Created default partition table with %zu partitions", m_partitions.size());
            return true;
        }
        ...
        bool PartitionTable::writeToFlash()
        {
            ...
            {
                ESP_LOGE(TAG, "Partition table too large: %zu bytes", table_size);
                return false;
            }
            ...
            ESP_LOGD(TAG, "Successfully wrote %zu partitions to flash", m_partitions.size());
            return true;
        }
        ...
        esp_partition_info_t* PartitionTable::getPartition(size_t index) const
        {
            if (index >= m_partitions.size())
            {
                ESP_LOGE(TAG, "Partition index out of bounds: %zu", index);
                return nullptr;
            }
